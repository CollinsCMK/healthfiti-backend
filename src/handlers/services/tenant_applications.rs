...
use uuid::Uuid;
use crate::{handlers::services::tenants::ApiResponseDTO, utils::{api_response::ApiResponse, http_client::ApiClient, jwt::get_logged_in_user_claims}};
...
pub async fn get_all_tenant_applications() -> Result<ApiResponse, ApiResponse> {
    let api = ApiClient::new();
    let endpoint = "tenant_applications";
...
pub async fn get_tenant_application_data(
    ...
    let api = ApiClient::new();
    let endpoint = format!("tenant_applications/check/{}/{}", claims.tenant_pid.unwrap(), claims.application_pid.unwrap());
...
pub async fn get_tenant_application_by_id(
    pid: Uuid,
...
pub async fn create_tenant_application(
    ...
    let response: ApiResponseDTO<CreateTenantApplicationResponse> = api
        .call("tenant_applications/create", &None, Some(&json_value), Method::POST)
        .await
        .map_err(|err| {
            log::error!("Failed to create tenant application: {}", err);
            ApiResponse::new(500, json!({ "message": "Failed to create tenant application" }))
        })?;
...
pub async fn update_tenant_application(
    ...
    let response: ApiResponseDTO<()> = api
        .call(&format!("tenant_applications/edit/{}", pid), &None, Some(&json_value), Method::PUT)
        .await
        .map_err(|err| {
            log::error!("Failed to update tenant application: {}", err);
            ApiResponse::new(500, json!({ "message": "Failed to update tenant application" }))
        })?;
...
pub async fn destroy_tenant_application(pid: Uuid) -> Result<ApiResponse, ApiResponse> {
    let api = ApiClient::new();
    let response: ApiResponseDTO<()> = api
        .call(&format!("tenant_applications/soft-delete/{}", pid), &None, None::<&()>, Method::DELETE)
        .await
        .map_err(|err| {
            log::error!("Failed to destroy tenant application: {}", err);
            ApiResponse::new(500, json!({ "message": "Failed to destroy tenant application" }))
        })?;
...
pub async fn restore_tenant_application(pid: Uuid) -> Result<ApiResponse, ApiResponse> {
    let api = ApiClient::new();
    let response: ApiResponseDTO<()> = api
        .call(&format!("tenant_applications/restore/{}", pid), &None, None::<&()>, Method::PATCH)
        .await
        .map_err(|err| {
            log::error!("Failed to restore tenant application: {}", err);
            ApiResponse::new(500, json!({ "message": "Failed to restore tenant application" }))
        })?;
...
pub async fn delete_tenant_application(pid: Uuid) -> Result<ApiResponse, ApiResponse> {
    let api = ApiClient::new();
    let response: ApiResponseDTO<()> = api
        .call(&format!("tenant_applications/delete/{}", pid), &None, None::<&()>, Method::DELETE)
        .await
        .map_err(|err| {
            log::error!("Failed to delete tenant application: {}", err);
            ApiResponse::new(500, json!({ "message": "Failed to delete tenant application" }))
        })?;
